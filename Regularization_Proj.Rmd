...
- <span style="color:black; font-weight:bold"> Akaike Information Criterion (AIC):</span>

- <span style="color:black; font-weight:bold"> Bayesian Information Criterion (BIC):</span>

- <span style="color:black; font-weight:bold"> Adjusted $\boldsymbol{R^2}$:</span>
...
```{r, warning=FASLSE,message=FALSE}
AIC_df <- data.frame(
...
The observation that LASSO and Ridge outperform the stepwise methods (and OLS), and also that LASSO outperforms Ridge is in line with the recommendation of by Hastie, Tibshirani, and Friedman (CITE ESLR). Furthermore, there seem to be more modern methods that outperform LASSO and Ridge that follow a mixed integer optimization approach to choosing best subsets, as outlined by Bertsimas, King, Mazumder (CITE: Best Subset Selection via a Modern Optimization Lens).
